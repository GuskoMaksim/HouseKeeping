/*
  MHZ19_uart.cpp - MH-Z19 CO2 sensor library for ESP-WROOM-02/32(ESP8266/ESP32)
  or Arduino version 0.3
...
MHZ19_uart::MHZ19_uart(int rx, int tx) { begin(rx, tx); }
...
int MHZ19_uart::getStatus() { return getSerialData(STAT); }

boolean MHZ19_uart::isWarming() { return (getStatus() <= 1); }
...
int MHZ19_uart::getSerialData(MHZ19_DATA flg) {
    ...
    if (buf[0] == 0xff && buf[1] == 0x86 &&
        mhz19_checksum(buf) == buf[MHZ19_uart::RESPONSE_CNT - 1]) {
        co2 = buf[2] * 256 + buf[3];
        co2temp = buf[4] - 40;
        co2status = buf[5];
    } else {
